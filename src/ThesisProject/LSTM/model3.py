...
def evaluate_and_plot(y_true, y_pred, production_lag, time_steps, forecast_horizon, prediction_method, installed_capacity=None):
    ...
    def calculate_errors(y_true, y_pred, installed_capacity, forecast_horizon):
        ...
        with open(f'model/{folder_name}/errors_{prediction_method}_{forecast_horizon}.txt'.format(
    ...
    def plot_time_series(y_true, y_pred, forecast_horizon, time_steps):
        ...
        if forecast_horizon == 1:
            ...
            plt.legend()
            plt.grid(True)
            plt.savefig(f'model/{folder_name}/time_series_{prediction_method}_{forecast_horizon}.png')
            plt.close()
    ...
    def plot_correlation(y_true, y_pred, forecast_horizon):
        if forecast_horizon == 1:
            ...
            plt.legend()
            plt.grid(True)
            plt.savefig(f'model/{folder_name}/correlation_{prediction_method}_{forecast_horizon}.png')
            plt.close()
    ...
    def plot_metrics_vs_horizon(errors, forecast_horizon):
        ...
        plt.tight_layout()
        plt.savefig(f'model/{folder_name}/metrics_vs_horizon_{prediction_method}_{forecast_horizon}.png')
        plt.close()
    ...
    def plot_multiple_predictions_subplots(y_true, y_pred, forecast_horizon, folder_name, prediction_method):
        ...
        plt.savefig(f'model/{folder_name}/time_series_4subplots_{prediction_method}_{forecast_horizon}.png')
        plt.close()
...
def lstm_wind_power_forecast(wf_id, start_date, end_date, time_steps=1, batch_size=128, epochs=100, dropout_rate=0.25,
    ...
    # if os.path.exists(f'model/{folder_name}/{model_name}') == True:
    #     return None
    ...
    if os.path.exists(f'model/{folder_name}/{model_name}') == False:

        # print all the parameters in a fancy way
        with open(f'model/{folder_name}/model_summary.txt', 'w') as f:
        ...
        plt.savefig(f'model/{folder_name}/training_history.png')

        if save_model:
            model.save(f'model/{folder_name}/{model_name}')
            with open(f'model/{folder_name}/{scaler_name}', 'wb') as f:
                pickle.dump(scaler, f)
            print("Model saved.")
    else:
        model = load_model(f'model/{folder_name}/{model_name}')
        with open(f'model/{folder_name}/{scaler_name}', 'rb') as f:
            scaler = pickle.load(f)

    # if error file exists, continue
    if os.path.exists(f'model/{folder_name}/errors_{prediction_method}_{forecast_horizon}.txt') == True:
        return None
...
time_steps_list = [8, 4, 2, 1] # 1 == no time steps
...
forecast_horizon = 8 # How many hours to predict ahead
...
# ### Hyperparameters - Autoregressive 24-hour horizon INFERENCE
# number_of_cells_list = [[64, 32, 16], [128, 64, 32], [256, 128, 64], [512, 256, 128]]
                ...
                for epochs in epochs_list:
                    for learning_rate in learning_rate_list:
                        for dropout_rate in dropout_rate_list:
                            ...
                            if os.path.exists(f'model/{folder_name}/errors_{prediction_method}_{forecast_horizon}.txt') == True:
                                #os.remove(f'model/{folder_name}/errors_{prediction_method}_{forecast_horizon}.txt')
                                clear_session()
                                continue
